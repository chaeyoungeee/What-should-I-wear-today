...
async def user_info(ctx):
    if user_data.check_exist(ctx.author.id):
        hot_level[0], recommand[0], recommand[1] = user_data.user_save_info(ctx.author.id)
    else:
...
async def 어디(ctx):
    ...
    async def gangnam_callback(interaction):
        information[0] = "강남구"
        await interaction.response.send_message(embed=discord.Embed(title="출발시간과 귀가시간을 입력해주세요!", description="!언제 출발시간 귀가시간\n(ex)!언제 9 19"))
    ...
    async def jungrang_callback(interaction):
        information[0] = "중랑구"
        await interaction.response.send_message(embed=discord.Embed(title="출발시간과 귀가시간을 입력해주세요!", description="!언제 출발시간 귀가시간가\n(ex)!언제 9 19"))
...
@bot.command()
async def h(ctx):
    await ctx.send(embed=discord.Embed(title="오늘 추천해드린 옷은 어땠나요?", description=""))
